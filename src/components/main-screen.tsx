    ...
    <div className="flex h-screen bg-[#2D2D30] text-[#CCCCCC]">
      ...
      <div className={`bg-[#252526] flex flex-col transition-all duration-300 ease-in-out ${isCollapsed ? 'w-16' : 'w-64'}`}>
        ...
        <div className={`flex-grow overflow-y-auto ${isCollapsed ? 'hidden' : 'block'}`}>
          <div className="p-4 space-y-4">
            <div>
              <label htmlFor="search" className="block text-sm font-medium mb-1">Search Location</label>
              <Input id="search" placeholder="Enter location..." className="bg-[#3E3E42] border-[#3E3E42]" />
            </div>

            <div>
              <label htmlFor="layers" className="block text-sm font-medium mb-1">Show Layers</label>
              <div className="flex items-center">
                <Switch id="layers" checked={showLayers} onCheckedChange={setShowLayers} />
        ...
        <div className={`mt-auto p-4 space-y-2 ${isCollapsed ? 'hidden' : 'block'}`}>
      ...
      <div className="flex-1 relative">
        ...
        <div className="absolute bottom-4 right-4 flex flex-col items-center space-y-2 bg-[#252526] p-2 rounded-md">
          <Button variant="ghost" size="icon" onClick={() => setZoomLevel(Math.min(zoomLevel + 10, 100))}>
          ...
          <Slider
            ...
            className="h-32"
          />
          <Button variant="ghost" size="icon" onClick={() => setZoomLevel(Math.max(zoomLevel - 10, 0))}>
